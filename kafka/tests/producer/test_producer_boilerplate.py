...
def test_successfully_initialize(built_object):
    with mock.patch('kafka.producer.producer_boilerplate.SerializingProducer', autospec=True) as ProducerMock, \
            mock.patch('kafka.producer.producer_boilerplate.SchemaRegistryClient', autospec=True) as SchemaRegistryMock:
...
def test_fetch_avro_serialization_schema(built_object):
    ...
    with mock.patch.object(built_object, '_KafkaProducer__schema_registry_client') as mocked_schema_registry:
        mocked_schema_registry.get_latest_version.return_value = expected_return_value
        res = built_object.fetch_serialization_schema(SupportedSerializers.AVRO_SERIALIZER, expected_subject_name)
...
def test_fetch_string_serialization_schema(built_object):
    res = built_object.fetch_serialization_schema(SupportedSerializers.STRING_SERIALIZER, None)
    assert res is None


def test_build_string_serializer(built_object):
    with mock.patch('kafka.producer.producer_boilerplate.StringSerializer', autospec=True) as mock_string_serializer:
...
def test_build_avro_serializer(built_object):
    with mock.patch('kafka.producer.producer_boilerplate.AvroSerializer', autospec=True) as mock_avro_serializer, \
            mock.patch.object(built_object, '_KafkaProducer__schema_registry_client') as mocked_schema_registry:
        ...
        mock_avro_serializer.assert_called_once_with(mocked_schema_registry, expected_schema)
...
def test_asynchronous_send(built_object):
    ...
    with mock.patch.object(built_object, '_KafkaProducer__producer') as mocked_producer:
        built_object.asynchronous_send(expected_key, expected_value, expected_topic, expected_callback)
        mocked_producer.produce.assert_called_once_with(topic=expected_topic,
                                                        key=expected_key,
